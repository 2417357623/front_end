...
const { jsonData, handledTableData ,dependencyData,setHandledTableData} = deployStore
 ...
 const check = async() => {
  ...
  myApi.dependencyCheck(dInInfo).then(res => {
    ...
    nextTick().then(() => {
    setHandledTableData(deployStore.computeDependencyCheck(handledTableData.value))
  })
  })
}
